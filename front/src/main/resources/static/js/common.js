$(function () {
    var assetsPrefix = '//assets.lrts.me/web';
    var s_md5 = document.createElement("script");
    s_md5.setAttribute("type", "text/javascript");
    s_md5.setAttribute("src", assetsPrefix + '/assets/javascript/cryptojs/md5.js');
    var s_hmac = document.createElement("script");
    s_hmac.setAttribute("type", "text/javascript");
    s_hmac.setAttribute("src", assetsPrefix + '/assets/javascript/cryptojs/hmac-sha1.js');
    var s_des = document.createElement("script");
    s_des.setAttribute("type", "text/javascript");
    s_des.setAttribute("src", assetsPrefix + '/assets/javascript/cryptojs/tripledes.js');
    document.getElementsByTagName("head")[0].appendChild(s_md5);
    document.getElementsByTagName("head")[0].appendChild(s_hmac);
    document.getElementsByTagName("head")[0].appendChild(s_des);
    ...
    $.yyting = {
        ...
        bindPhoneBox: function () {
            $.dialog({
                title: '',
                content: $('#bindPhoneDiv').html(),
                complete: function () {
                    this.el.find('#cancel').on('click', function () {
                        $.dialog.close();
                    })
                    this.el.find('#toBindPhone').on('click', function (e) {
                        e.preventDefault();
                        location.href = '/setting/bind?from=comment';
                    })
                }
            })
        },
        checkBindPhone: function () {
            ...
            if (user && this.checkLogin()) {
                ...
                return user.isPhoneBind == 1
            }
            return false
        },
        checkLogin: function () {
            var result = document.cookie.match(new RegExp("(^| )sid=([^;]*)(;|$)")) || []
        ...
        initUser: function () {
            ...
            // 修复第三方登陆浏览器localstorage没有存储user的问题
            if ($.yyting.checkLogin()) {
                $.ajax({
                    url: "/user/loginUserInfo.do",
                    type: "get",
                    dataType: "json",
                    data: {},
                    success: function (data) {
                        $.yyting.localStorage('user', JSON.stringify(data.data))
                        user = $.yyting.localStorage('user')
                        user = JSON.parse(user);
    ...
    /*申请完本*/
    $(document).on('click', '.album-f-applyend', function () {
        var $this = $(this),
            albumId = $this.attr('albumnId');
        $.dialog({
            title: '',
            content: $('#album-dialog-template').html(),
            defined: function () {
                $.yyting.ajax({
                    login: true,
                    url: '/upload/updateContentState.do',
                    type: 'post',
                    dataType: 'json',
                    data: {
                        id: albumId,
                    ...
                    success: function (data) {
                        if (data.status == 'success') {
                            ...
                            var html = '<span class="album-f-status album-f-isapplying">完本受理中...</span>';
